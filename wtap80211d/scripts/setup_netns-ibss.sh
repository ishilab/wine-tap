...
for wiphy in ${!interfaces[@]}; do
    ...
    ip netns exec "${netspaces[$wiphy]}" ip addr add ${ipaddrs[$wiphy]}/${NETWORK_MASK} brd "${BROADCAST_ADDR}" dev "${interfaces[$wiphy]}" || exit $?
    ...
    ip netns exec "${netspaces[$wiphy]}" ${IW} dev ${interfaces[$wiphy]} set type ibss || exit $?
